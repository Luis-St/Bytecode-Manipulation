...
public class ImplementedTransformer extends BaseClassTransformer {
	...
	private static final class ImplementedVisitor extends ContextBasedClassVisitor {
		...
		private void validateMethod(@NotNull Method ifaceMethod, @NotNull Class targetClass) {
			...
			if (targetMethod == null) {
				throw createReport("Method annotated with @Implemented must be implemented in target class", ifaceMethod.getOwner(), signature).addDetailBefore("Interface", "Target Class", targetClass.getType()).exception();
			}
			if (!targetMethod.is(TypeAccess.PUBLIC)) {
				throw createReport("Method annotated with @Implemented must be public in target class", ifaceMethod.getOwner(), signature).addDetailBefore("Interface", "Target Class", targetClass.getType())
