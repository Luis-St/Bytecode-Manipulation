...
public class InjectorTransformer extends BaseClassTransformer {
	...
	private static final String REPORT_CATEGORY = "Injector Implementation Error";
	...
	private static final class InjectorClassVisitor extends ContextBasedClassVisitor {
		...
		private InjectorClassVisitor(@NotNull ClassWriter writer, @NotNull Type type, @NotNull Runnable markModified, @NotNull Map<String, List<String>> lookup) {
		...
		private void validateMethod(@NotNull Method ifaceMethod, @NotNull Class targetClass) {
			...
			CrashReport report = CrashReport.create(REPORT_CATEGORY).addDetail("Interface", ifaceMethod.getOwner()).addDetail("Injector", signature);
			
			if (!ifaceMethod.is(TypeAccess.PUBLIC)) {
				throw report.exception("Method annotated with @Injector must be public");
			}
			if (ifaceMethod.is(TypeModifier.ABSTRACT)) {
				throw report.exception("Method annotated with @Injector must be default implemented");
			}
			if (ifaceMethod.getExceptionCount() > 0) {
				throw report.addDetail("Exceptions", ifaceMethod.getExceptions()).exception("Method annotated with @Injector must not throw exceptions");
			}
			
			Method existingMethod = targetClass.getMethod(ifaceMethod.getSignature(SignatureType.FULL));
			if (existingMethod != null) {
				throw report.addDetail("Existing Method", existingMethod.getSignature(SignatureType.DEBUG)).exception("Target class of injector already has method with same signature");
			}
			...
			if (possibleMethod.isEmpty()) {
				throw report.addDetail("Method", injectName).addDetail("Possible Methods", targetClass.getMethods(this.getRawInjectName(injectName)).stream().map(Method::toString).toList())
					.exception("Could not find method specified in injector");
			}
			if (possibleMethod.size() > 1) {
				throw report.addDetail("Method", injectName).addDetail("Possible Methods", possibleMethod.stream().map(Method::toString).toList()).exception("Found multiple possible methods for injector");
			}
			
			Method method = possibleMethod.getFirst();
			if (!ifaceMethod.is(TypeModifier.STATIC) && method.is(TypeModifier.STATIC)) {
				throw report
					.addDetail("Method", method.getSignature(SignatureType.DEBUG)).exception("Method annotated with @Injector is declared none-static, but specified a static method");
			}
			if (ifaceMethod.getParameterCount() > 0) {
				for (Parameter parameter : ifaceMethod.getParameters().values()) {
					if (!parameter.isAnnotatedWith(THIS) && !parameter.isAnnotatedWith(LOCAL)) {
						throw report.addParameterDetails(parameter).exception("Parameter of injector must be annotated with @This or @Local");
					}
					if (method.is(TypeModifier.STATIC) && parameter.isAnnotatedWith(THIS)) {
						throw report.addParameterDetails(parameter).addDetail("Method", method.getSignature(SignatureType.DEBUG)).exception("Parameter of injector cannot be annotated with @This, because the specified method is static");
					}
				}
			}
			if (method.returns(VOID)) {
				if (!ifaceMethod.returns(VOID) && !ifaceMethod.returns(BOOLEAN)) {
					throw report.addDetail("Injector Return Type", method.getReturnType()).addDetail("Method", method.getSignature(SignatureType.DEBUG))
						.exception("Method annotated with @Injector specified a method which returns void, but injector method does not return void or boolean (cancellation)");
				}
				...
				if (!ifaceMethod.returns(methodWrapper)) {
					throw report.addDetail("Injector Return Type", methodWrapper).addDetail("Method", method.getSignature(SignatureType.DEBUG)).addDetail("Method Return Type", method.getReturnType())
						.exception("Method annotated with @Injector specified a method which returns a primitive type, but injector method does not return the corresponding wrapper type");
				}
			} else if (!ifaceMethod.returns(VOID) && !ifaceMethod.returns(method.getReturnType())) {
				throw report.addDetail("Injector Return Type", ifaceMethod.getReturnType()).addDetail("Method", method.getSignature(SignatureType.DEBUG)).addDetail("Method Return Type", method.getReturnType())
					.exception("Method annotated with @Injector must either return void or the same type as the specified method");
			}
			...
			if (!scanner.visitedTarget()) {
				throw report.addDetail("Scanned Class", targetClass.getType()).addDetail("Method", method.getSignature(SignatureType.DEBUG)).exception("Could not find method specified in injector during scan of its own class");
			}
			...
			if (lambdaMethod != null && !ifaceMethod.is(TypeModifier.STATIC)) {
				throw report.addDetail("Method", method.getSignature(SignatureType.DEBUG)).addDetail("Lambda", lambdaMethod.getSignature(SignatureType.DEBUG))
					.exception("Method annotated with @Injector is declared none-static, but specified a lambda expression");
			}
			if (line == -1) {
				throw report.addDetail("Method", method.getSignature(SignatureType.DEBUG)).addDetail("Lambda", lambdaMethod).addDetail("Target", annotation.get("value")).addDetail("Target Type", annotation.get("type"))
					.addDetail("Target Mode", annotation.getOrDefault("mode")).addDetail("Target Ordinal", annotation.getOrDefault("ordinal")).addDetail("Target Offset", annotation.getOrDefault("offset"))
					.removeNullValues(true).exception("Could not find target in method body of method specified in injector");
			}
		...
		public MethodVisitor visitMethod(int access, @NotNull String name, @NotNull String descriptor, @Nullable String signature, String @Nullable [] exceptions) {
			...
			if (this.injects.containsKey(name + descriptor)) {
				this.markModified();
				return new InjectorMethodVisitor(new LocalVariablesSorter(access, descriptor, visitor), this.injects.get(name + descriptor));
			}
			return visitor;
		}
		...
		private @NotNull String getInjectName(@NotNull Method ifaceMethod) {
			...
			if (methodName.startsWith("injector")) {
	...
	private static final class InjectorMethodVisitor extends LabelTrackingMethodVisitor {
		
		private final Map</*Line Number*/Integer, List<InjectData>> injects = new HashMap<>();
		private int lastLine = -1;
		
		private InjectorMethodVisitor(@NotNull LocalVariablesSorter visitor, @NotNull List<InjectData> injects) {
			super(visitor);
				...
				for (int i = this.lastLine + 1; i < line; i++) {
					if (this.injects.containsKey(i)) {
						...
						if (injects.size() == 1) {
							...
							throw CrashReport.create("Injector was not instrumented correctly, because no label was found", REPORT_CATEGORY).addDetail("Line", i).addDetail("Interface", inject.ifaceMethod().getOwner())
								.addDetail("Injector", inject.ifaceMethod().getSignature(SignatureType.DEBUG)).addDetail("Method", inject.method().getSignature(SignatureType.DEBUG)).addDetail("Lambda Method", inject.lambdaMethod()).removeNullValues(true).exception();
						} else {
							CrashReport report = CrashReport.create("Injects were not instrumented correctly, because no label was found", REPORT_CATEGORY);
							
							List<Map<String, Object>> details = new ArrayList<>();
							for (InjectData inject : injects) {
								Map<String, Object> detail = new HashMap<>();
								detail.put("Interface", inject.ifaceMethod().getOwner());
								detail.put("Injector", inject.ifaceMethod().getSignature(SignatureType.DEBUG));
		...
		private void instrumentInject(@NotNull InjectData inject) {
			CrashReport report = CrashReport.create(REPORT_CATEGORY).addDetail("Injector", inject.ifaceMethod().getSignature(SignatureType.DEBUG)).addDetail("Method", inject.method().getSignature(SignatureType.DEBUG));
			
			if (inject.lambdaMethod() != null) {
				if (!inject.lambdaMethod().returns(VOID)) {
					throw report.addDetail("Lambda Method", inject.lambdaMethod().getSignature(SignatureType.DEBUG)).exception("Injector into a lambda expression must return void");
				}
				
				this.instrumentInjectAsListener(inject.ifaceMethod(), inject.lambdaMethod());
				inject.ifaceMethod().getAnnotation(INJECTOR).getValues().put("lambda", true);
				...
				throw report.addDetail("Line", inject.line()).exception("Unknown how to implement injector, tried as listener, callback and cancellation but failed");
			}
		}
		...
		private void instrumentMethodCall(@NotNull Method ifaceMethod, @NotNull Method method) {
			...
			for (Parameter parameter : ifaceMethod.getParameters().values()) {
				this.mv.visitVarInsn(parameter.getType().getOpcode(Opcodes.ILOAD), getLoadIndex("injector", parameter, ifaceMethod, method, this.getScopeIndex()));
			}
