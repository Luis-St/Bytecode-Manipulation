...
public class InvokerTransformer extends BaseClassTransformer {
	...
	private static final class InvokerVisitor extends ContextBasedClassVisitor {
		...
		private void validateMethod(@NotNull Method ifaceMethod, @NotNull Class targetClass) {
			CrashReport report = CrashReport.create(REPORT_CATEGORY).addDetail("Interface", ifaceMethod.getOwner()).addDetail("Invoker", ifaceMethod.getSignature(SignatureType.DEBUG));
			
			if (!ifaceMethod.is(TypeAccess.PUBLIC)) {
				throw report.exception("Method annotated with @Invoker must be public");
			}
			if (ifaceMethod.is(TypeModifier.STATIC)) {
				throw report.exception("Method annotated with @Invoker must not be static");
			}
			if (!ifaceMethod.is(TypeModifier.ABSTRACT)) {
				throw report.exception("Method annotated with @Invoker must not be default implemented");
			}
			
			Method existingMethod = targetClass.getMethod(ifaceMethod.getSignature(SignatureType.FULL));
			if (existingMethod != null) {
				throw report.addDetail("Existing Method", existingMethod.getSignature(SignatureType.DEBUG)).exception("Target class of invoker already has method with same signature");
			}
			...
			if (possibleTargets.isEmpty()) {
				throw report.addDetail("Possible Targets", targetClass.getMethods(this.getRawInvokerName(invokerTarget)).stream().map(Method::toString).toList()).exception("Could not find target method for invoker");
			}
			if (possibleTargets.size() > 1) {
				throw report.addDetail("Possible Targets", possibleTargets.stream().map(Method::toString).toList()).exception("Found multiple possible targets for invoker");
			}
			
			Method targetMethod = possibleTargets.getFirst();
			if (targetMethod.is(TypeAccess.PUBLIC)) {
				throw report.addDetail("Target Method", targetMethod.getSignature(SignatureType.DEBUG)).exception("Target method of invoker is public, no invoker required");
			}
			if (!targetMethod.is(ifaceMethod.getType())) {
				throw report.addDetail("Target Method", targetMethod.getSignature(SignatureType.DEBUG)).exception("Invoker method signature does not match target method signature");
			}
			
			if (!Objects.equals(targetMethod.getSignature(SignatureType.GENERIC), ifaceMethod.getSignature(SignatureType.GENERIC))) {
				throw report.addDetail("Target Method", targetMethod.getSignature(SignatureType.GENERIC)).exception("Invoker method signature does not match target method signature");
			}
		...
		private @NotNull String getInvokerName(@NotNull Method ifaceMethod) {
			...
			if (methodName.startsWith("invoker")) {
				return Utils.uncapitalize(methodName.substring(7));
			}
			return methodName;
		}
