...
public class ClassContentScanner extends BaseClassVisitor {
	...
	public @NotNull MethodVisitor visitMethod(int access, @NotNull String name, @NotNull String descriptor, @Nullable String signature, String @Nullable [] exception) {
		...
		MethodData method = new MethodData(this.owner, name, Type.getType(descriptor), signature, TypeAccess.fromAccess(access), MethodType.fromName(name), TypeModifier.fromMethodAccess(access), new HashMap<>(), new ArrayList<>(), exceptions, new HashMap<>(), new Mutable<>());
