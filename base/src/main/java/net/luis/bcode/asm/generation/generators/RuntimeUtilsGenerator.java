package net.luis.bcode.asm.generation.generators;
...
import static net.luis.bcode.asm.generation.Generations.*;
...
public class RuntimeUtilsGenerator extends Generator {
	...
	private void generateGetActualType(@NotNull ClassVisitor cv) {
		MethodVisitor mv = cv.visitMethod(Opcodes.ACC_PUBLIC | Opcodes.ACC_STATIC, "getActualType", "(Ljava/lang/String;Ljava/lang/String;I)" + ACTUAL_TYPE.getDescriptor(), null, null);
		//region Labels
		Label start = new Label();
		Label end = new Label();
		//endregion
		...
		mv.visitMethodInsn(Opcodes.INVOKESTATIC, SIGNATURE_UTILS.getInternalName(), "parseSignatureParameters", "(Ljava/util/Map;Ljava/lang/String;)Ljava/util/List;", false);
		mv.visitVarInsn(Opcodes.ILOAD, 2);
		mv.visitMethodInsn(Opcodes.INVOKEINTERFACE, "java/util/List", "get", "(I)Ljava/lang/Object;", true);
		mv.visitTypeInsn(Opcodes.CHECKCAST, ACTUAL_TYPE.getInternalName());
		mv.visitInsn(Opcodes.ARETURN);
		mv.visitLabel(end);
